...
function nTimeSteps = setParams (inputpath,codepath,listterm,Nx,Ny,Nr)  
  ...
  simTime = 40*t1year; %%% Simulation time  
...
%   nIter0 = 880894; %%% Initial iteration (pick up from a previous run)
  ...
  use_local_polynya = false;
  if (use_local_polynya)
    Lpolynya = 50*m1km;
    Xpolynya = 0*m1km;
  else
    Lpolynya = 50*m1km;
...
%   h(:,end) = 0;  
  ...
  Y_h = (Y-Ys)/Ws;
  if (use_trough)
    h_trough = H_trough * (1-(2*(X-X_trough)/W_trough).^2).*heaviside(W_trough/2-abs(X-X_trough));    
    if (slope_trough)      
      if (H_trough <= 0)
        z_topog = Zs + h_trough;  
...
%           tau_y(j) = 1;
  ...
  for n=1:nForcingPeriods
    for j=1:Ny      
      for i=1:Nx
        if (use_local_polynya)          
          if (yy(j)^2 + (xx(i)-Xpolynya)^2 <= Lpolynya^2)
            salt_flux(i,j,n) = salt_amp;             
          end
        else
          if (yy(j) <= Lpolynya)          
            salt_flux(i,j,n) = salt_amp;             
...
%         mskS(:,j,1) = 0;
  ...
  layers_bounds = 1000 + [27.5:0.005:27.78 27.782:0.002:27.88 27.8804:0.0004:27.91];
  ...
  {...
    ...
    'THETA','VVELTH','WVELTH', ... %%% Temperature and transport
    'SALT','VVELSLT','WVELSLT', ... %%% Salt and transport
    'THETASQ','SALTSQ','THSLT', ...  %%% For full TEM with neutral density
    'UVELTH','UVELSLT' %%% Needed for 3D topographic configurations
%     'GM_PsiY', ... %%% GM bolus transport streamfunction    
  };
  
  numdiags_avg = length(diag_fields_avg);  
  diag_freq_avg = 2.5*t1year;
  diag_phase_avg = 0;    
