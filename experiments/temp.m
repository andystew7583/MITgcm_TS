...
ND_avg = squeeze(mean(gamma_tavg));
psi_ND = psi_zgam;
load MOC_output/TS_tau0.075_Ws75_Hs500_Ymax25_Ly450_Sflux2.5e-3_res1km_hifreq_MOC_ND1.mat
ND1_avg = squeeze(mean(gamma_tavg));
psi_ND1 = psi_zgam;
[psi_maxint_ND psi_maxtot_ND] = calcPsiInt_OceMod(psi_ND,ND_avg,gam_bdy,Ny,polynyaidx,spongeidx,Nr,zzf);
[psi_maxint_ND1 psi_maxtot_ND1] = calcPsiInt_OceMod(psi_ND1,ND1_avg,gam_bdy,Ny,polynyaidx,spongeidx,Nr,zzf);
...
figure(10);
contourf(YY_psi,ZZ_psi,psie_G,-1:0.05:1)
colorbar
caxis([-1 1]);
...
vt_mean = vv.*tt;
vt_eddy = vt-vt_mean;
vt_tot = sum(sum(vt.*repmat(reshape(delR,[1 1 Nr]),[Nx Ny 1]).*repmat(delX',[1 Ny Nr]).*hFacS,3),1);
vt_m_tot = sum(sum(vt_mean.*repmat(reshape(delR,[1 1 Nr]),[Nx Ny 1]).*repmat(delX',[1 Ny Nr]).*hFacS,3),1);
vt_e_tot = sum(sum(vt_eddy.*repmat(reshape(delR,[1 1 Nr]),[Nx Ny 1]).*repmat(delX',[1 Ny Nr]).*hFacS,3),1);

figure
plot(yy,vt_tot*rho0*Cp)
hold on
plot(yy,vt_m_tot*rho0*Cp)
plot(yy,vt_e_tot*rho0*Cp)
plot(yy,0*yy,'k--')
hold off
