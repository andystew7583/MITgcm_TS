%%%
%%% plotShelfSensitivity.m
%%%
%%% Plots sensitivity of the solution to continental shelf depth.
%%%
...
N_Hs_vals = length(Hs_vals);
EKE_slope = zeros(N_Hs_vals,1);
T_aabw = zeros(N_Hs_vals,1);
S_aabw = zeros(N_Hs_vals,1);
Psi_sw = zeros(N_Hs_vals,1);
Psi_cdw = zeros(N_Hs_vals,1);
Psi_aabw = zeros(N_Hs_vals,1);
for i=1:N_Hs_vals
  ...
  [T_aabw(i),S_aabw(i),Psi_sw(i),Psi_cdw(i),Psi_aabw(i)] = ...
               theory(T_sw,S_sw,T_cdw,S_cdw,Sigma,Y_cdw,Y_aabw,Z_cdw,Z_aabw, ...
                                  -tau_val,rho0,f0,g,alpha0,beta0,zb(i),sb(i),EKE_slope(i),c);                                                     
...
%   Psi_cdw(i) = 0.01*sqrt(EKE_slope(i))*ll_rh(i)*abs(sb(i));
end
...
plot(Hs_vals,-psimax(:,psimax_idx),'go-','MarkerSize',10,'LineWidth',2);
hold on;
plot(Hs_vals,-psimax_CDW(:,psimax_idx),'ro-','MarkerSize',10,'LineWidth',2);
plot(Hs_vals,-psimax_SW(:,psimax_idx),'bo-','MarkerSize',10,'LineWidth',2);
plot(Hs_vals,-psimax_SW(:,psimax_idx),'bo-','MarkerSize',10,'LineWidth',2);
plot(Hs_vals,-psimax_CDW(:,psimax_idx),'ro-','MarkerSize',10,'LineWidth',2);
plot(Hs_vals,-psimax(:,psimax_idx),'go-','MarkerSize',10,'LineWidth',2);
hold off;
...
clf;
axes('FontSize',fontsize);
plot(Hs_vals,-vt_tot(:,shelfidx)*Lx*rho0*Cp/1e9,'go-','MarkerSize',10,'LineWidth',2);
hold on;
plot(Hs_vals,-vt_m_tot(:,shelfidx)*Lx*rho0*Cp/1e9,'bo-','MarkerSize',10,'LineWidth',2);
plot(Hs_vals,-vt_e_tot(:,shelfidx)*Lx*rho0*Cp/1e9,'ro-','MarkerSize',10,'LineWidth',2);
hold off;
xlabel('Shelf depth (m)','interpreter','latex');
...
clf;
axes('FontSize',fontsize);
[ax,h1,h2] = plotyy(Hs_vals,aabw_temp(:,aabwidx),Hs_vals,aabw_salt(:,aabwidx),'plot');
xlabel('Shelf depth (m)','interpreter','latex');
set(h1,'LineStyle','-');
set(h2,'LineStyle','-');
set(h1,'Marker','s');
...
set(ax(2),'YLim',[34.62 34.64]);
set(ax(2),'YTick',[34.62:0.005:34.64]);
...
%%% Smaller plot with just CDW overturning sensitivity
handle = figure(4);
set(handle,'Position',framepos);
clf;
axes('FontSize',fontsize);
...
xlabel('Shelf depth (m)','interpreter','latex');
ylabel('Transport (Sv)','Rotation',90,'interpreter','latex');
axis([min(Hs_vals)-50 max(Hs_vals)+50 0 0.15]);
set(gca,'Position',plotloc);
...
clf;
axes('FontSize',fontsize);
plot(Hs_vals,-psimax_CDW(:,psimax_idx),'rs-','MarkerSize',6,'LineWidth',1.5,'MarkerFaceColor','r');
hold on;
plot(Hs_vals,Psi_cdw*Lx/1e6,'ko--','MarkerSize',6,'LineWidth',1.5,'MarkerFaceColor','k');
hold off;
xlabel('Slope width (km)','interpreter','latex');
ylabel('$F_{\mathrm{CDW}}$ (Sv)','Rotation',90,'interpreter','latex');
axis([min(Hs_vals)-10 max(Hs_vals)+10 0 0.4]);
set(gca,'Position',plotloc);
